...
const THRESHOLDS: Record<string, { maxHours: number; label: string }> = {
  ...
  uc_with_buyer: { maxHours: 720, label: 'Stale closing (30+ days)' },
};
...
export async function runDispoAccountability(event: GunnerEvent): Promise<void> {
  ...
  try {
    ...
    for (const opp of staleOpps) {
      ...
      if (!isDryRun()) {
        await taskBot(opp.contactId, {
          ...
          auditLog({ agent: AGENT_ID, contactId: opp.contactId, action: 'taskBot:failed', result: 'error', reason: err?.message });
        });

        await tagBot(opp.contactId, ['dispo-stale']).catch(err => {
          auditLog({ agent: AGENT_ID, contactId: opp.contactId, action: 'tagBot:failed', result: 'error', reason: err?.message });
        });
    ...
    auditLog({ agent: AGENT_ID, contactId: 'system', action: 'agent:crashed', result: 'error', reason: err?.message });
  }
}
