...
export async function runCallCoaching(event: CallEvent): Promise<CallScore | null> {
  ...
  const score = await classifierBot.scoreCall(callId, contactId, transcript, playbook);

  if (!isDryRun()) {
    await noteBot(contactId, templateBot.buildNote('coaching:summary', { callId, summary: score.summary }));
  }
