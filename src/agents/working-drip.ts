...
export async function runWorkingDrip(event: DripEvent): Promise<void> {
  ...
  try {
    ...
    if (dayOffset >= ghostedDay && !event.lastRealContact) {
      await emit({ kind: 'lead.ghosted', tenantId, contactId, opportunityId, metadata: { dayOffset } }).catch(err => {
        auditLog({ agent: AGENT_ID, contactId, action: 'emit:lead.ghosted:failed', result: 'error', reason: err?.message });
      });
    }
    ...
    if (!isDryRun()) {
      await smsBot(contactId, template).catch(err => {
        auditLog({ agent: AGENT_ID, contactId, action: 'smsBot:failed', result: 'error', reason: err?.message });
      });
    }

    await emit({ kind: 'drip.step-sent', tenantId, contactId, opportunityId, metadata: { step: stepIndex, dayOffset } }).catch(err => {
      auditLog({ agent: AGENT_ID, contactId, action: 'emit:drip.step-sent:failed', result: 'error', reason: err?.message });
    });
