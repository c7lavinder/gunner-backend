...
Return top matches sorted by score descending.`,
    ...
    if (!isDryRun()) {
      ...
      for (const buyer of topBuyers.slice(0, 5)) {
        await tagBot(buyer.buyerId, ['matched-deal']).catch(err => {
          auditLog({ agent: AGENT_ID, contactId: buyer.buyerId, action: 'tagBot:failed', result: 'error', reason: err?.message });
        });
      }
    }
    ...
    await emit({
      ...
      auditLog({ agent: AGENT_ID, contactId, action: 'emit:buyer.matched:failed', result: 'error', reason: err?.message });
    });
