...
export async function runResponseAgent(event: SmsEvent): Promise<void> {
  ...
  try {
    ...
    if (classification === 'dnc') {
      await emit({ kind: 'lead.dnc', tenantId, contactId, opportunityId, metadata: { message } }).catch(err => {
        auditLog({ agent: AGENT_ID, contactId, action: 'emit:lead.dnc:failed', result: 'error', reason: err?.message });
      });
    ...
    if (currentStage && offerStages.includes(currentStage)) {
      await emit({ kind: 'offer.reply', tenantId, contactId, opportunityId, metadata: { message, classification } }).catch(err => {
        auditLog({ agent: AGENT_ID, contactId, action: 'emit:offer.reply:failed', result: 'error', reason: err?.message });
      });
    ...
    if (currentStage && followUpStages.includes(currentStage)) {
      await emit({ kind: 'followup.closer', tenantId, contactId, opportunityId, metadata: { message, classification } }).catch(err => {
        auditLog({ agent: AGENT_ID, contactId, action: 'emit:followup.closer:failed', result: 'error', reason: err?.message });
      });
      auditLog({ agent: AGENT_ID, contactId, opportunityId, action: 'response:followup-closer', result: 'success', durationMs: Date.now() - start });
      return;
    }

    await emit({ kind: 'lead.responded', tenantId, contactId, opportunityId, metadata: { message, classification } }).catch(err => {
      auditLog({ agent: AGENT_ID, contactId, action: 'emit:lead.responded:failed', result: 'error', reason: err?.message });
    });

    await memoryWriterBot.recordAction('classification-corrections', { contactId, message }, { classification }, tenantId).catch((err) => {
