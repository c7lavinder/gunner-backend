...
export async function runAptPrep(event: AppointmentEvent): Promise<void> {
  ...
  try {
    ...
    if (cancelled) {
      ...
      await noteBot(contactId, templateBot.buildNote('apt:cancelled', { type })).catch(err => {
    ...
    await fieldBot(contactId, { [fAptId]: appointmentId, [fAptTime]: appointmentTime, [fAptType]: type }).catch(err => {
      auditLog({ agent: AGENT_ID, contactId, action: 'fieldBot:failed', result: 'error', reason: err?.message });
    });
    await noteBot(contactId, templateBot.buildNote('apt:prep', { type, appointmentTime: aptTime.toLocaleString() })).catch(err => {
      auditLog({ agent: AGENT_ID, contactId, action: 'noteBot:failed', result: 'error', reason: err?.message });
    });
