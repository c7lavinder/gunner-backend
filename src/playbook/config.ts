...
export function loadConfig(): PlaybookConfig {
  _config = {
    ...
    stages: {
      ...
      pendingApt:    process.env.STAGE_PENDING_APT ?? '',
      walkthrough:   process.env.STAGE_WALKTHROUGH ?? '',
      offerApt:      process.env.STAGE_OFFER_APT ?? '',
      appointment:   process.env.STAGE_APPOINTMENT ?? '',
      madeOffer:     process.env.STAGE_MADE_OFFER ?? '',
      ...
      agreementNotClosed: process.env.STAGE_AGREEMENT_NOT_CLOSED ?? '',
      sold:          process.env.STAGE_SOLD ?? '',
      doNotWant:     process.env.STAGE_DO_NOT_WANT ?? '',
